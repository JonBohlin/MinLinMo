...
// g++ MinLinMo.cpp -lpthread -std=c++17  -O3 -lgsl -lblas -lpthread -march=armv8-a
...
class VariableSelector{
    ...
    VariableSelector(float cor, float R2, float resCor){
        ...
        std::cout<<"Number of predictors correlating with outcome:"<<sortedCorList.size()<<std::endl;
        // Time model fitting
    ...
    void compute_neon_correlation( Submatrix &predMatrix, const int offset ){
        ...
        // const int numLanes = 4; // Only 4 on Neon
        ...
        float32x4_t _x, _y, _xbar, _xdiv, _xdiff, _sumyx, _sumyxsq, _rootsumyxsq, _rho;
        float32x4_t _xdiffsq, _ydiff, _ydiffsq, _sumydiffsq;
        _xdiv = vdupq_n_f32( (float) rowSize );
        _x = vdupq_n_f32( 0.0f );
        _xbar = vdupq_n_f32( 0.0f );
        _sumyx = vdupq_n_f32( 0.0f );
        _sumyxsq = vdupq_n_f32( 0.0f );
        _sumydiffsq = vdupq_n_f32( sumydiffsq );
        ...
        for(int k=0; k < avxCycles; k++){
            ...
            for(int i=0; i < rowSize; i++){
                ...
                _xbar = vaddq_f32( _xbar, _x );
            }
            _xbar = vdivq_f32( _xbar, _xdiv );
            ...
            for(int i=0; i < rowSize; i++){
                ...
                _ydiff = vdupq_n_f32( ydiff[i] );
                _xdiff = vsubq_f32( _x, _xbar );
                _xdiffsq = vmulq_f32( _xdiff, _xdiff );
                _sumyx = vmlaq_f32( _sumyx, _ydiff, _xdiff );
                _sumyxsq = vmlaq_f32( _sumyxsq, _sumydiffsq, _xdiffsq);
            }
            _rootsumyxsq = vsqrtq_f32( _sumyxsq );
            _rho = vdivq_f32( _sumyx, _rootsumyxsq );
            _x = vdupq_n_f32( 0.0f );
            _xbar = vdupq_n_f32( 0.0f );
            _sumyx = vdupq_n_f32( 0.0f );
            _sumyxsq = vdupq_n_f32( 0.0f );
    ...
    void compute_correlations( universal::FloatMatrix &matrix ){
        ...
        for( int i=0; i < submatrices.size(); i++ )
            threads.push_back(std::thread(&VariableSelector::compute_neon_correlation, this, std::ref(submatrices[i]), offsets[i]));
...
int main(int argv, char* args[]){
    ...
    std::string matrixFile, outcomeFile;
    std::cout<<"============================="<<std::endl;
    std::cout<<"Simple variable selector v1.0"<<std::endl;
    std::cout<<"============================="<<std::endl;
    std::cout<<"Number of arguments:"<<argv<<std::endl;
    if( argv <= 1 ){
        std::cout<<"Need at least two file filenames\n"<<std::endl;
        std::cout<<"./- <outcome file> <matrix file>  [min correlation=0.1] [min Delta R2=0.01] [min residual correlation=0.1]"<<std::endl;
        exit( 0 );
    }
