...
// To compile on Windows, install GSL on Visual Studio:
...
class VariableSelector{
    ...
    void compute_avx_correlation( Submatrix &predMatrix, const int offset ){
        ...
        int predMatrixIndex;
        size_t rowSize = predMatrix.numRows();
        size_t colSize = predMatrix.numCols();
        corIndexElement CI;

        const int numLanes = 8; // Each lane is a 32 bit floating point
        size_t avxCycles = colSize / numLanes;
        const int avxRestCycle = colSize % numLanes;
        float floatvec[numLanes];
        ...
        __m256 _x, _xbar, _xdiv, _xdiff, _sumyx, _sumyxsq, _rootsumyxsq, _rho;
        __m256 _xdiffsq, _ydiff, _sumydiffsq;
        ...
        for(int k=0; k < avxCycles; k++){
            ...
            for(int i=0; i < rowSize; i++){
                for(int j=0; j < numLanes; j++){
                    ...
                    floatvec[j] = predMatrix(i, predMatrixIndex);
                }
                _x = _mm256_load_ps(floatvec);
            ...
            _mm256_store_ps(floatvec, _rho);
            
            for(int j=0; j < roundCount; j++){
                if( fabs(floatvec[j]) > cor_threshold ){
                    CI.cor = fabs(floatvec[j]);
